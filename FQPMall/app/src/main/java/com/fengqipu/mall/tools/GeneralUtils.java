...
//                .into(iv);
     ...
     * @param defaultImg
     */
    public static void setBlurImageViewWithUrl(final Context context, final String url, ImageView iv, int defaultImg) {
        Glide.with(context)
                .load(url)
//                .placeholder(defaultImg)
                .crossFade()
...
//    public static void setImageViewWithUrl(final Context context, final String url, ImageView iv, int width, int height, int defaultImg) {
//        CMLog.e("sub", "newurl=" + url);
//        if (GeneralUtils.isNotNullOrZeroLenght(url) && GeneralUtils.isFileExist(context, url)) {
//            String newurl = url;
//            if (url.contains("?")) {
//                newurl = url.split("\\?")[0];
//            }
...
//            File file = new File(FileSystemManager.getTemporaryPath(context), bitmapName);
//            Picasso.with(context).load(file).error(R.drawable.default_bg).into(iv);
//        } else {
//            if (GeneralUtils.isNotNullOrZeroLenght(url)) {//getTemporaryPath
//                if (width == 0 || height == 0) {
...
//                                    try {
//                                        if (source != null) {  //缓存图片
//                                            FileSystemManager.saveBitmap2(context, source, url);
//                                        }
//                                    } catch (Exception e) {
//                                        e.printStackTrace();
//                                    }
...
//                            .placeholder(defaultImg)
//                            .config(Bitmap.Config.RGB_565)
//                            .resize(DisplayUtil.dip2px(context, width), DisplayUtil.dip2px(context, height))
//                            .centerCrop()
//                            .transform(new Transformation() {
//                                @Override
//                                public Bitmap transform(Bitmap source) {
